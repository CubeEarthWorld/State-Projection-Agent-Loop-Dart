...
| Compaction | `compaction.trigger_ratio` (default `0.75`, `0` off) | One extra model call folds the history before the verbatim point into `WorkingState` as a schema-validated, grounding-checked JSON delta (`state_folded` keeps the pre-fold state). |
